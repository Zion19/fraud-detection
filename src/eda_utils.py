...
def plot_class_distribution(df, target_col, title, palette=None, save_path=None):
    ...
    summary = pd.DataFrame(
        {"count": counts.values, "percent_%": pcts.values},
        index=(
            ["Normal (0)", "Fraud (1)"]
            if len(counts) == 2
            else [str(x) for x in counts.index]
        ),
    )
    display(summary)
    ...
    pie_labels = (
        ["Normal", "Fraud"] if len(counts) == 2 else [str(x) for x in counts.index]
    )
    ax2.pie(
        counts.values,
        labels=pie_labels,
        autopct="%1.3f%%",
        colors=[palette.get(x, "#888888") for x in counts.index],
...
def plot_correlation_heatmap(df, cols, title, cmap="coolwarm", save_path=None):
    """Plots a correlation matrix heatmap for numeric columns"""
